...
# ----------------
# 상수 정의
# 접속 정보는 환경변수로 받는다. 코드에 비밀번호를 적어 두면 저장소에 그대로 남는다.
#   Windows  set DB_PASSWORD=비밀번호
#   Linux    export DB_PASSWORD=비밀번호
DB_USER = os.environ.get("DB_USER", "root")
DB_PASSWORD = os.environ.get("DB_PASSWORD", "")
DB_HOST = os.environ.get("DB_HOST", "localhost")
DB_NAME = os.environ.get("DB_NAME", "한빛무역")

if not DB_PASSWORD:
    raise SystemExit("DB_PASSWORD 환경변수를 설정하고 다시 실행하세요.")

DB_URL = f"mysql+pymysql://{DB_USER}:{quote_plus(DB_PASSWORD)}@{DB_HOST}/{DB_NAME}?charset=utf8mb4"
SALES_QUERY = """
SELECT 고객.지역 AS 지역,
        CASE
