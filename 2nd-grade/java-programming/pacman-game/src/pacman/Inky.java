// Inky는 수줍고 예측할 수 없는 성격을 가짐
// 특징: Blinky와 Pac-Man의 위치를 고려하여 예측할 수 없는 경로로 이동
// 공포 상태일 때는 Pac-Man과 멀어지고, 그 외에는 Blinky의 위치를 참조하여 목표를 설정
// 무작위 이동 확률을 높여 예측이 어렵도록 만듦
...
public class Inky extends Ghost {
    private Ghost blinky; // Blinky 유령을 참조하여 Inky의 이동 결정
    private double randomnessFactor = 0.3; // 30% 확률로 무작위 이동

    public Inky(int startX, int startY, String leftImagePath, String rightImagePath, String scaredImagePath, Ghost blinky) {
        super(startX, startY, leftImagePath, rightImagePath, scaredImagePath, "Inky");
        this.blinky = blinky;
    }

    /**
     * Inky의 이동 메서드
     * 공포 상태일 때는 Pac-Man으로부터 멀어지고, 그렇지 않을 때는 Blinky와 Pac-Man의 위치를 기반으로 A*를 통해 최단 경로 이동
     *
    ...
    public void move(int[][] board, int pacmanX, int pacmanY, int pacmanDirectionX, int pacmanDirectionY, List<Ghost> otherGhosts, int pacmanScore) {
        ...
        } else if (Math.random() < randomnessFactor) {
            // 무작위로 이동하여 예측 불가한 움직임 추가
            randomMove(board, otherGhosts);
        } else {
            // Blinky와 Pac-Man의 위치를 이용한 목표 설정
            int intermediateX = pacmanX + pacmanDirectionX * 2;
            int intermediateY = pacmanY + pacmanDirectionY * 2;
            int targetX = (blinky.getX() + intermediateX) / 2;
            int targetY = (blinky.getY() + intermediateY) / 2;
    ...
    /**
     * A* 알고리즘을 사용해 Inky가 목표 위치로 가는 최단 경로를 찾음
     *
     * @param board 게임 보드 배열
